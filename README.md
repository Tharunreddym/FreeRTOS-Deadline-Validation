...
Under normal operation, processing completes under the 80 ms deadline. During overload injection, processing intentionally exceeds the deadline at around 120 ms, allowing the monitor task to detect and log a deadline miss. Once overload clears, the processor returns under deadline and the system logs recovery.

## Resume Bullet

Developed a FreeRTOS Task Deadline Validation System on STM32 with producer, processor, and monitor tasks, validating 100 ms producer timing, 80 ms processor deadline enforcement, overload-induced deadline miss detection, recovery behavior, UART diagnostics, and Python-based HIL test reporting.
